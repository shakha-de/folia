...
export default function GamificationWidgets({ gamification, leaderboard }: GamificationWidgetsProps) {
    return (
        <div className="flex flex-col gap-8">
            {/* Achievements Widget */}
            <div className="bg-[#1c2a20] rounded-xl border border-[#28392b] p-6">
            ...
            {/* Leaderboard Widget */}
            <div className="bg-[#1c2a20] rounded-xl border border-[#28392b] overflow-hidden">
                ...
                <div className="flex flex-col">
                    {leaderboard.map((entry: LeaderboardEntry) => (
                        <div key={entry.rank} className={`flex items-center gap-3 p-4 ${entry.isCurrentUser ? 'bg-primary/10 border-l-4 border-l-primary hover:bg-primary/15' : 'border-b border-[#28392b] hover:bg-[#28392b]'} transition-colors`}>
                            <div className={`size-6 flex items-center justify-center font-bold rounded text-xs ${entry.rank === 1 ? 'bg-yellow-500/20 text-yellow-500' : entry.rank === 2 ? 'bg-gray-400/20 text-gray-400' : entry.rank === 3 ? 'bg-orange-400/20 text-orange-400' : 'text-[#9db9a1]'}`}>
                                {entry.rank}
                            </div>
                            <div
