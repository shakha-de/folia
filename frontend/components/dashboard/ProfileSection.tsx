...
export default function ProfileSection({ user, gamification }: ProfileSectionProps) {
    return (
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Profile Info */}
            <div className="lg:col-span-2 bg-[#1c2a20] rounded-xl p-6 border border-[#28392b] shadow-lg relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-64 h-64 bg-primary/5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 pointer-events-none"></div>
                <div className="flex flex-col sm:flex-row gap-6 items-center sm:items-start relative z-10">
                    <div
                        className="bg-center bg-no-repeat bg-cover rounded-full size-24 sm:size-32 border-4 border-[#28392b] shrink-0 shadow-md bg-stone-700"
                        data-alt="Detailed user profile picture"
                    >
                        {/* Placeholder for avatar if no image */}
                        <div className="flex items-center justify-center w-full h-full text-white text-4xl font-bold opacity-50">
                            {user.username.charAt(0).toUpperCase()}
                        </div>
                    </div>
                    <div className="flex flex-col justify-center text-center sm:text-left flex-1">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-1">
                            <h1 className="text-white text-2xl font-bold leading-tight">Welcome back, {user.username}</h1>
                        ...
                        <p className="text-[#9db9a1] text-base mb-4">Current Rank: <span className="text-white font-semibold">{gamification.rank}</span> • {gamification.location}</p>
                        {/* Progress Bar Integrated */}
                        <div className="w-full max-w-md bg-[#111812] rounded-full h-3 mb-2 mx-auto sm:mx-0">
                        ...
                        <p className="text-[#9db9a1] text-xs font-medium">{gamification.progress}% to <span className="text-primary">{gamification.nextRank}</span></p>
                    </div>
                </div>
            </div>
